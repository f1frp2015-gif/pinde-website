...
export default function SupplyPage({ locale, content }: Props) {
  ...
  const sectionLabel = locale === "ru" ? "Форматы поставки" : "Supply formats";
  const crumbs = breadcrumbJsonLd([
    { name: homeLabel, url: `https://pindesys.com/${locale}` },
    { name: sectionLabel, url: `https://pindesys.com/${locale}/supply` },
  ]);
          ...
          <div className="hidden lg:grid grid-cols-[160px_repeat(3,minmax(0,1fr))] border border-line">
            ...
            {(["from", "local", "fit"] as const).map((key, ri) => (
              <div key={key} className="contents">
                <div className={`border-t border-line p-5 text-[11px] font-bold text-[#DAAF37] ${ri % 2 ? "bg-surface" : "bg-obsidian"}`}>
              ...
              <article key={f.level} className="border border-line bg-obsidian">
                ...
                <div className="p-5">
                  ...
                  <dl className="mt-5 grid gap-5">
                    <div><dt className="text-[10px] font-bold uppercase text-[#DAAF37]">{content.labels.chongqing}</dt><dd className="mt-2 text-[12px] leading-[1.65] text-warm">{f.from}</dd></div>
                    <div><dt className="text-[10px] font-bold uppercase text-[#DAAF37]">{content.labels.local}</dt><dd className="mt-2 text-[12px] leading-[1.65] text-warm">{f.local}</dd></div>
                    <div><dt className="text-[10px] font-bold uppercase text-[#DAAF37]">{content.labels.fit}</dt><dd className="mt-2 text-[12px] leading-[1.65] text-warm">{f.fit}</dd></div>
                  </dl>
                </div>
              </article>
            ))}
