        ...
        <div className={container}>
          ...
          <div className="hidden lg:grid grid-cols-[160px_repeat(3,minmax(0,1fr))] border border-line">
            ...
            {content.formats.map((f: { level: string; tag: string; title: string; from: string; local: string; fit: string }) => (
              <div key={f.level} className="border-l border-line bg-[#102F45] p-5 text-white">
            ...
            {(["from", "local", "fit"] as const).map((key, ri) => (
              <div key={key} className="contents">
                <div className={`border-t border-line p-5 text-[11px] font-bold text-[#0E527D] ${ri % 2 ? "bg-surface" : "bg-obsidian"}`}>
              ...
              <article key={f.level} className="border border-line bg-obsidian">
                ...
                <div className="p-5">
                  ...
                  <dl className="mt-5 grid gap-5">
                    <div><dt className="text-[10px] font-bold uppercase text-[#0E527D]">{content.labels.chongqing}</dt><dd className="mt-2 text-[12px] leading-[1.65] text-warm">{f.from}</dd></div>
                    <div><dt className="text-[10px] font-bold uppercase text-[#0E527D]">{content.labels.local}</dt><dd className="mt-2 text-[12px] leading-[1.65] text-warm">{f.local}</dd></div>
                    <div><dt className="text-[10px] font-bold uppercase text-[#0E527D]">{content.labels.fit}</dt><dd className="mt-2 text-[12px] leading-[1.65] text-warm">{f.fit}</dd></div>
                  </dl>
                </div>
              </article>
            ))}
          </div>
          <p className="mt-6 border-l-4 border-[#0E527D] bg-obsidian px-5 py-4 text-[12px] leading-[1.7] text-muted">{content.note}</p>
        </div>
      </section>
      <section className="py-[89px] bg-obsidian">
