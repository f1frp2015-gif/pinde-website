    ...
    <>
      ...
      <section className="py-[89px] bg-surface">
        <div className={`${container} grid gap-[13px] lg:grid-cols-2`}>
          {content.platforms.map((platform: { code: string; label: string; title: string; description: string; features: string[]; href: string; cta: string }) => (
            <article key={platform.code} className="bg-obsidian border border-line rounded-[2px] overflow-hidden flex flex-col">
              <div className="bg-[#0E527D] p-6 flex items-center justify-between">
                <span className="text-white text-[22px] font-extrabold">{platform.code}</span>
                <span className="text-white/60 text-[10px] font-bold uppercase tracking-[0.08em]">{platform.label}</span>
              </div>
              <div className="p-6 sm:p-8 flex flex-col flex-1">
                ...
                <Link
                  ...
                  className="mt-6 inline-flex items-center gap-2 px-[34px] py-4 bg-red text-white text-[11px] font-medium tracking-[3px] uppercase rounded-[1px] hover:brightness-90 transition-all w-fit"
                >
                  {platform.cta}
                  <ArrowRight size={14} />
                </Link>
        ...
        <div className={`${container} mt-6`}>
          <p className="border-l-4 border-[#F0B429] bg-[#1A1A0A] px-5 py-4 text-[12px] leading-[1.7] text-muted">
            {content.note}
          </p>
        </div>
      </section>
