...
export default function Header({ locale }: HeaderProps) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-[#0D2440] text-white shadow-[0_12px_32px_rgba(13,36,64,0.18)]">
        ...
        <div className="mx-auto flex h-full max-w-[1280px] items-center justify-between px-5 text-[10px] sm:px-8 lg:px-10">
          ...
          <div className="ml-4 flex shrink-0 items-center gap-4">
            <a
              ...
              className="hidden items-center gap-1.5 text-white/60 transition-colors hover:text-[#C7A154] sm:flex"
            >
              <Mail size={12} className="text-[#C7A154]" aria-hidden="true" />
              {BRAND.email}
            </a>
            <div className="flex items-center gap-2 border-l border-white/15 pl-4 font-bold">
              <Link
                ...
                hrefLang="en"
                lang="en"
                className={locale === "en" ? "text-[#C7A154]" : "text-white/45 hover:text-white"}
              ...
              <Link
                ...
                hrefLang="ru"
                lang="ru"
                className={locale === "ru" ? "text-[#C7A154]" : "text-white/45 hover:text-white"}
      ...
      <div className="border-b border-white/10">
        <div className="mx-auto flex h-16 max-w-[1280px] items-center justify-between px-5 sm:px-8 lg:h-[72px] lg:px-10">
          ...
          >
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="border-b-2 border-transparent py-2 text-[11px] font-semibold text-white/66 transition-colors hover:border-[#C7A154] hover:text-white"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Phone size={14} className="text-[#C7A154]" aria-hidden="true" />
              {BRAND.phone}
            </a>
            <Link
              href={`/${locale}/rfq`}
              className="hidden min-h-11 items-center bg-[#C7A154] px-5 text-[11px] font-bold text-[#0D2440] transition-[filter] hover:brightness-105 sm:inline-flex"
            >
              {content.headerCta}
            </Link>
            <MobileNav
